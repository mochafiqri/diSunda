...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public List<Question> getQuestionMode(String mode) {
        ...
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM Question WHERE KET=1 ORDER BY Random() LIMIT %d", limit), null);
    ...
    public List<Question1> getQuestion0() {
        ...
        try {
            c = db.rawQuery(String.format("SELECT * FROM Question WHERE KET=0 ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
                String quest = c.getString(c.getColumnIndex("Pertanyaan"));
                String CorrectAnswer = c.getString(c.getColumnIndex("CorrectAnswer"));

                Question1 question = new Question1(Id, quest,CorrectAnswer);
                listQuestion.add(question);
            }
            while (c.moveToNext());
            c.close();
